...
namespace RayTracer
{
    constexpr int MARBLES_SIZE = 600;

    static double generatedMarbleData[MARBLES_SIZE][6] = {
	...
	{
		World world;
		world.lights.push_back(PointLight(Color(1.5, 1.5, 1.5), point(-4, 4, -3)));

		Camera camera(image.width, image.height, MATH_PI / 3);
		camera.transform = viewTransform(point(0, 7, -5), point(0, 1, 2), vector(0, 1, 0));
		...
		lamp.setTransform(translation(-5, 6, -3) * scaling(0.75, 0.75, 0.75));
		world.objects.push_back(&lamp);

		CheckerPattern floorPattern(Color(0.8, 0.7, 1), Color(0.15, 0.1, 0.3));
		floorPattern.setTransform(translation(0, 0.5, 0) * rotationY(MATH_PI / 4));
		Plane floor;
        floor.material.specular = 0.8;
        floor.material.diffuse = 1;
        floor.material.specular = 1;
        ...
        for (int i = 0; i < MARBLES_SIZE; i++)
        {
            ...
            marbles[i].material.color =
                Color(generatedMarbleData[i][0], 
                    generatedMarbleData[i][1], 
                    generatedMarbleData[i][2]);

            marbles[i].setTransform(
                translation(
                    generatedMarbleData[i][3], 
                    generatedMarbleData[i][5], 
                    generatedMarbleData[i][4]) *
                scaling(
                    generatedMarbleData[i][5], 
                    generatedMarbleData[i][5], 
                    generatedMarbleData[i][5]));

            marbles[i].material.diffuse = 0.8;
            marbles[i].material.specular = 0;
